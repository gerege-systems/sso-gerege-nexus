...
**Gerege SSO** 是一个构建于
[Gerege Nexus](https://github.com/gerege-systems/open-gerege-nexus) 之上的开源平台，
用于统一管理公共与私营机构的用户、认证、权限与系统访问。平台以**蒙古语为默认语言**，
并直接对接蒙古国国家数字基础设施（DAN、E-ID、XYP / ХУР）。

公民或员工使用国家数字身份验证**一次**，即可访问其获授权的所有应用，无需重复登录。
第三方系统通过 OAuth2 / OIDC 接入该会话，无需各自维护密码库。

*Nexus* 意为连接点：机构、服务、工作流、系统、用户与数据在此交汇。Gerege SSO 正是这一
连接的**身份与访问层**——确认来者是谁，并决定其可以访问什么。真正决定一次部署形态的，
是运行其上的模块。
